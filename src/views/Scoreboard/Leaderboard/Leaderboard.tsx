...
export default function Leaderboard({ className }: Props) {
    ...
    const [maxSize, setMaxSize] = useState(Math.floor(Math.min(window.innerWidth * 0.2, window.innerHeight * 0.4)));
    ...
    useEffect(() => {
        const handleResize = () => setMaxSize(Math.floor(Math.min(window.innerWidth * 0.2, window.innerHeight * 0.4)));
